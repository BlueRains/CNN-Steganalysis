"""Create and test a ResNet50 CNN."""
...
def preprocess_data(image_paths: list[str], labels: list[int]):
...
def main():
    """Train a ResNet50 model and test."""
    # Paths to datasets
    ...
    # logging.info the metrics
    logging.info("ResNet50 Classification Report:")
    logging.info(classification_report(y_true, y_pred_classes))

    logging.info("ResNet50 Confusion Matrix:")
    conf_matrix = confusion_matrix(y_true, y_pred_classes)
    logging.info(conf_matrix)

    # Accuracy
    accuracy = np.mean(y_pred_classes == y_true)
    logging.info("ResNet50 Accuracy: %d", accuracy)

    # Precision
    precision = precision_score(y_true, y_pred_classes, average="binary")
    logging.info("ResNet50 Precision: %d", precision)

    # Recall
    recall = recall_score(y_true, y_pred_classes, average="binary")
    logging.info("ResNet50 Recall: %d", recall)

    # F1 Score
    f1 = f1_score(y_true, y_pred_classes, average="binary")
    logging.info("ResNet50 F1 Score: %d", f1)

    # Area Under the Receiver Operating Characteristic curve (AUC-ROC)
    roc_auc = roc_auc_score(y_true, y_pred_classes)
    logging.info("ResNet50 AUC-ROC: %d", roc_auc)
